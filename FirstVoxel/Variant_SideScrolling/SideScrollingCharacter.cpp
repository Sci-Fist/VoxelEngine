...
/**
 *  @note This class is an EXAMPLE implementation of a side-scrolling mechanic.
 *  The primary active controller in use is the Voxel Character Controller.
 */
...
ASideScrollingCharacter::ASideScrollingCharacter()
{
	...
	// Configure camera boom for side scrolling
	if (GetCameraBoom())
	{
		GetCameraBoom()->TargetArmLength = 300.0f;
		GetCameraBoom()->bUsePawnControlRotation = false; // Lock rotation
		GetCameraBoom()->SetRelativeRotation(FRotator(0.0f, -90.0f, 0.0f));
	}
...
void ASideScrollingCharacter::DoMove(float Right, float Forward)
{
...
void ASideScrollingCharacter::MultiJump()
{
	...
	if (!bHasWallJumped)
	{
		...
		{
			// Coyote time: walked off a ledge within MaxCoyoteTime seconds — grant a regular jump.
			Jump();
...
void ASideScrollingCharacter::CheckForSoftCollision()
{
	...
	if (OutHit.GetActor())
	{
		...
		// [Expert Bugfix] Reset soft collision after 0.35s so the player doesn't ignore platforms indefinitely
		GetWorld()->GetTimerManager().SetTimer(SoftCollisionTimer, this, &ASideScrollingCharacter::ResetSoftCollision, 0.35f, false);
	}
}
...
void ASideScrollingCharacter::ResetSoftCollision()
{
...
void ASideScrollingCharacter::Tick(float DeltaTime)
{
	...
	// Apply biome-dependent movement physics (e.g. slippery desert sand).
	if (AVoxelWorld* VW = FindAndCacheVoxelWorld())
	{
		const FVector Loc = GetActorLocation();
		const FVoxelBiomeWeightMap Weights =
			FVoxelBiomeManager::GetBiomeWeightsStatic(Loc.X, Loc.Y, VW->GetEffectiveConfig());

		// Desert sand reduces braking friction so the character slides slightly when stopping.
		const float DesertWeight   = Weights.GetWeight(EVoxelBiome::Desert);
		const float TargetFriction = FMath::Lerp(1.0f, 0.4f, DesertWeight);
		GetCharacterMovement()->BrakingFrictionFactor = TargetFriction;
	}
}
