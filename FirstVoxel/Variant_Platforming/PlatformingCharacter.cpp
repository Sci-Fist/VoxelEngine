...
APlatformingCharacter::APlatformingCharacter()
{
	...
	// Cache the designer-set gravity scale so EndDash() can restore it exactly,
	// even if a subclass changes it in BeginPlay.
	DefaultGravityScale = GetCharacterMovement() ? GetCharacterMovement()->GravityScale : 2.5f;
...
{
	// Restore the gravity scale captured at construction time.
	// Hardcoding 2.5 here would silently break if the designer changed GravityScale.
...
void APlatformingCharacter::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
	...
	{
		// Jumping (Override base to support multi-jump)
...
void APlatformingCharacter::Tick(float DeltaTime)
{
	...
	// Only update when standing on the ground
	if (!GetCharacterMovement()->IsMovingOnGround()) return;

	// Apply biome-dependent movement physics.
	// Platforming uses GroundFriction (not BrakingFrictionFactor) and interpolates for smooth biome transitions.
	if (AVoxelWorld* VW = FindAndCacheVoxelWorld())
	{
		...
		// Default grip; desert biome reduces traction for a loose-sand feel.
		const float DesertWeight  = Weights.GetWeight(EVoxelBiome::Desert);
		const float TargetFriction = FMath::Lerp(4.0f, 1.5f, DesertWeight);

		// FInterpTo smooths the transition when crossing biome blend boundaries.
		GetCharacterMovement()->GroundFriction =
			FMath::FInterpTo(GetCharacterMovement()->GroundFriction, TargetFriction, DeltaTime, 5.0f);
	}
}
