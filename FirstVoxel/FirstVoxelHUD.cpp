// FirstVoxelHUD.cpp
//
// FIX N12 — Decomposed 400-line DrawHUD() monolith into focused helpers:
//     DrawLoadingScreen  — chunk-map grid + progress bar
//     DrawTitleScreen    — main menu + world options panel
//     DrawGameplayHUD    — crosshair + control labels
//     DrawToolWheel      — radial tool selector overlay
//     DrawStatsOverlay   — FPS + coordinates (top-right corner)
//
// FIX N16 — DefaultSlotNames / MaxSaveSlots now come from
//            UI/VoxelSaveSlotConfig.h instead of being pulled in via
//            the full VoxelPauseMenu.h include.
...
namespace
{
    // Inline helper shared across all screen draw methods
...
// ============================================================
//  DrawHUD — dispatch to focused helpers
// ============================================================
void AFirstVoxelHUD::DrawHUD()
{
    ...
    // Pause menu takes full priority
    if (PauseMenu && PauseMenu->IsOpen()) { PauseMenu->Draw(); return; }

    UFont* Font = GEngine ? GEngine->GetSmallFont() : nullptr;

    // Cache voxel world once per session
    if (!CachedVoxelWorld)
    {
        TArray<AActor*> WA;
    ...
    if (bShowLoadingScreen && CachedVoxelWorld) { DrawLoadingScreen(Font); return; }
    if (bShowTitleScreen)                        { DrawTitleScreen(Font);   return; }

    // Gameplay HUD (crosshair + controls + tool wheel + stats)
...
void AFirstVoxelHUD::DrawLoadingScreen(UFont* Font)
{
    ...
    DrawRect(FLinearColor(0.25f,0.85f,0.35f,1.f), BX,BY,BW*W->GetGenerationProgress(),BH);

    // Status text
    const int32 Head=W->GetQueueHead(), Total=W->GetQueueCount();
    FString Status = FString::Printf(TEXT("Chunks: %d / %d"), Head, Total);
    if (W->IsWaitingForInitialSpawn())
        Status = FString::Printf(TEXT("Preparing Spawn Area: %d / %d"), Head, Total);
    else if (Head>=Total && Total>0) Status = TEXT("Finalizing... Ready shortly.");
    else if (Total<=0)               Status = TEXT("Requesting coordinates...");
    float SW,SH; GetTextSize(Status,SW,SH,Font,1.f);
    DrawText(Status, FLinearColor::White, CX-SW*0.5f, BY+BH+8.f, Font, 1.f);

    // Chunk-map miniature grid
    const float BoxSz=14.f, BoxPad=3.f;
    const int32 Radius=11;
    ...
    for (int32 cx=-Radius; cx<=Radius; ++cx)
    {
        ...
        DrawRect(Col, GridX+(cx+Radius)*(BoxSz+BoxPad), GridY+(cy+Radius)*(BoxSz+BoxPad), BoxSz, BoxSz);
    }
...
void AFirstVoxelHUD::DrawTitleScreen(UFont* Font)
{
    ...
    // Ensure cursor visible (FInputModeGameAndUI keeps LMB in game chain for WasInputKeyJustPressed)
    APlayerController* PC = GetOwningPlayerController();
    if (PC && !PC->bShowMouseCursor)
    {
        FInputModeGameAndUI M; M.SetLockMouseToViewportBehavior(EMouseLockMode::DoNotLock); M.SetHideCursorDuringCapture(false);
        PC->SetInputMode(M); PC->SetShowMouseCursor(true);
    }
    ...
    if (PC)
    {
        ...
        if (bConfirm && !bShowWorldOptions)
        {
            ...
            { bShowTitleScreen=false; bShowLoadingScreen=true; Restore(); CachedVoxelWorld->GenerateWorld(); }
...
// ============================================================
//  DrawStatsOverlay — FPS + coordinates
// ============================================================
void AFirstVoxelHUD::DrawStatsOverlay(UFont* Font)
{
    ...
    // Coordinates + altitude (below FPS)
