...
void AVoxelWorld::UpdateChunkStreaming() {
  ...
  // Double-incrementing here caused streaming to fire at half the intended
  // interval. This function is only called when the timer has already elapsed —
  // just reset it.
  StreamingTimer = 0.f;

  // --- ⚡ Optimization: Skip building streaming volumes if player is
  // stationary --- This optimization prevents unnecessary streaming
  // calculations when the player isn't moving
  ...
  if (FVector::DistSquared(PlayerPos, LastSkyAltPos) >
      SkyAltSnapDist * SkyAltSnapDist) {
    LastSkyAltPos = PlayerPos;

    const FVoxelBiomeManager::FWeightsAndHeight Wh =
        FVoxelBiomeManager::GetWeightsAndSurfaceHeightStatic(
            PlayerPos.X, PlayerPos.Y, Config);
    const float PlayerSurfH = Wh.SurfaceHeight;

    const float HeightNormSky =
        FMath::Clamp(PlayerSurfH / SC.MaxTerrainReference, 0.f, 1.f);
    const float RoughnessNormSky = FMath::Clamp(
        Wh.Weights.GetRoughness() / SC.RoughnessReference, 0.f, 1.f);
    const float TerrainStrSky =
        FMath::Clamp(HeightNormSky * 1.5f + RoughnessNormSky * 0.8f, 0.f, 1.f);
    CachedCurvedH = FMath::Pow(HeightNormSky, 2.5f);
    CachedCurvedR = FMath::Pow(RoughnessNormSky, 2.0f);
    const float AltBase = FMath::Lerp(
        SC.MinAltitudeAboveTerrain, SC.BaseAltitudeAboveTerrain, TerrainStrSky);

    // STREAMING BUG FIX: mirror the GetSkylandColumnCache altitude formula
    // exactly.
    //
    // Previous code:  CachedSkyAltWorld = SurfH + AltBase  (~3800 cm for flat
    // terrain) Actual shard altitude in GetSkylandColumnCache:
    //   DecoupledHeight = Lerp(AbsoluteSkyAnchor=15000, CenterHeight,
    //   CellShardT) SkyAlt = DecoupledHeight + AltBase + CellShardT *
    //   HeightAltitudeBonus
    //
    ...
    static constexpr float AbsoluteSkyAnchor =
        15000.f; // must match GetSkylandColumnCache
    const float ShardT =
        FMath::SmoothStep(0.f, SC.ShardTransitionStrength, TerrainStrSky);
    const float DecoupledH =
        FMath::Lerp(AbsoluteSkyAnchor, PlayerSurfH, ShardT);
  ...
  // FIX: Reduced render distance to minimize boundary crossings and chunk
  // instability This reduces the frequency of chunks crossing streaming
  // boundaries
  const int32 OptimizedRenderDistanceXY = FMath::Max(
      1, RenderDistanceXY - 1); // Reduced by 1 to minimize boundary crossings
  ...
  // FIX: Parallelize heavy noise lookups to prevent GameThread stall during
  // character movement.
  ParallelFor(NumCols, [&](int32 Index) {
    ...
    const FVoxelBiomeManager::FWeightsAndHeight Wh =
        FVoxelBiomeManager::GetWeightsAndSurfaceHeightStatic(ColX, ColY,
                                                             Config);
    GroundZCenters[Index] =
        FMath::RoundToInt(Wh.SurfaceHeight / ChunkWorldSize);
  });

  // Populate Desired set sequentially on the GameThread from the parallel-built
  // index array
  for (int32 Index = 0; Index < NumCols; ++Index) {
    ...
    for (int32 z = -RenderDistanceZ; z <= RenderDistanceZ; ++z) {
      Desired.Add(
          FIntVector(PlayerCoord.X + x, PlayerCoord.Y + y, GroundZCenter + z));
    }
  }
  ...
  // 2. Skylands Pass: Separate volume for floating islands with different
  // render distance

  // This uses a larger range to maintain distance visibility without
  // overloading ground chunks

  // Use continuous Z bounds (no rounding) to prevent churn when SkyAltWorld
  // varies smoothly
  const float HalfThickWorld = HalfThickCm; // world units (cm)
  const int32 SkyZMin =
      FMath::FloorToInt((SkyAltWorld - HalfThickWorld) / ChunkWorldSize);
  const int32 SkyZMax =
      FMath::CeilToInt((SkyAltWorld + HalfThickWorld) / ChunkWorldSize);

  for (int32 z = SkyZMin; z <= SkyZMax; ++z)
    for (int32 y = -SkylandsRenderDistanceXY; y <= SkylandsRenderDistanceXY;
         ++y)
      for (int32 x = -SkylandsRenderDistanceXY; x <= SkylandsRenderDistanceXY;
           ++x) {
        // FIX: Cylindrical culling
        if (x * x + y * y <=
            SkylandsRenderDistanceXY * SkylandsRenderDistanceXY) {
  ...
  for (const FIntVector &C : ToRemove) {
    DestroyChunk(C);
    // FIX: Prune EmptyChunks when chunks leave range so the set doesn't grow
    // unboundedly. Previously EmptyChunks was only cleared on ClearWorld(),
  ...
  //   2. Enforce consistency: if any neighbor has higher detail (lower LOD
  //   number),
  ...
  //
  // The neighbor check uses 6-directional adjacency
  // (up/down/north/south/east/west). This guarantees that the entire loaded
  // volume is LOD-uniform except at the outermost boundary where lower-detail
  // chunks may appear.
  //
  // Performance: O(N) where N = number of loaded chunks. Each chunk checks up

  // to 6 neighbors. For typical view distances (5-8 chunks), this is
  // negligible.
  //
  // NOTE: The hysteresis bands (L1ISq, L1OSq, etc.) prevent rapid LOD
  // flip-flopping when a chunk sits near a distance threshold. The factor
  // of 1.10 provides a 10% deadband to stabilize transitions.
  //
  ...
  for (auto &It : LoadedChunks) {
    ...
    if (IsValid(Chunk)) {
      ...
      // sqrt() per loaded chunk per streaming update (was ~500 sqrts every
      // 0.25s).
      FVector ChunkPos =
      ...
      const float L1OSq =
          LOD1Distance * LOD1Distance * HysteresisFactor * HysteresisFactor;
      const float L2ISq = LOD2Distance * LOD2Distance;
      const float L2OSq =
          LOD2Distance * LOD2Distance * HysteresisFactor * HysteresisFactor;

      int32 TargetLOD = Chunk->LOD;
      if (Chunk->LOD < 2 && DistSq > L2OSq)
        TargetLOD = 2;
      else if (Chunk->LOD > 1 && DistSq < L2ISq)
        TargetLOD = 1;
      else if (Chunk->LOD < 1 && DistSq > L1OSq)
        TargetLOD = 1;
  ...
  // Single pass may not catch all inconsistencies when multiple chunks need to
  // adjust
  bool bChanged = true;
  int32 PassCount = 0;
  ...
  for (const FIntVector &C : UniqueMerged) {
    ...
    const int32 DistSq =
        Local.X * Local.X + Local.Y * Local.Y + Local.Z * Local.Z;
...
void AVoxelWorld::EnforceLODConsistency() {
  ...
  const float CloseRangeThreshold =
      ChunkSize * VoxelSize * 3.0f; // 3 chunks distance
